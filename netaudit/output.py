...
from .models import RiskLevel, ScanReport, ScanStatus
...
def _print_tls_summary(report: ScanReport, nc: bool) -> None:
    ...
    for r in tls:
        c = r.cert
        if c is None:  # tls_ports() ja filtra, guard existe para o type checker
            continue
        if c.expired:
...
def _print_footer(report: ScanReport, nc: bool) -> None:
    ...
    risk_parts = []
    if crit_c:
        risk_parts.append(_c(f"{crit_c} CRITICAL", _RED, _BOLD, no_color=nc))
    if high_c:
        risk_parts.append(_c(f"{high_c} HIGH", _RED, no_color=nc))
    if med_c:
        risk_parts.append(_c(f"{med_c} MEDIUM", _YELLOW, no_color=nc))
    if low_c:
        risk_parts.append(_c(f"{low_c} LOW/INFO", _CYAN, no_color=nc))
...
def export_markdown(report: ScanReport, path: str) -> None:
    ...
    a("> **FOR AUTHORISED SECURITY ASSESSMENT USE ONLY**")
    a("")
    a("## Scan Metadata")
    a("")
    a("| Property | Value |")
    ...
    if open_ports:
        a("## Open Ports")
        a("")
        a("| Port | Protocol | Service | RTT (ms) | Banner |")
        a("|---|---|---|---|---|")
        for r in open_ports:
            server = r.http_headers.get("server", r.http_headers.get("Server", ""))
    ...
    if tls_ports:
        a("## TLS Certificate Details")
        a("")
        for r in tls_ports:
            c = r.cert
            if c is None:  # idem: tls_ports() ja garante o cert
                continue
    ...
    a("---")
    a("*Report generated by [NetAudit](https://github.com/JulianoVinceCampos/netaudit)."
      " Authorised use only.*")
    a("")
...
def make_progress_callback(
    total: int, quiet: bool, no_color: bool
